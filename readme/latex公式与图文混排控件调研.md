...
##最优方案
- HtmlTextview+JLaTexMath  二次开发
- HtmlTextView：用于解析H5标签，但是仅支持部分标签，可扩展自己的标签。核心是依靠textview的span来显示。

##替代方案
- MathView：用于处理解析不了的公式的渲染方案。思路为将mathview渲染之后获取view的图片，加载到textview中。

##扩展h5标签
- 在HtmlTagHandler文件中，handleTag函数用于标签解析，可扩展自己的标签。参考（“table” "formula"）标签的解析代码。核心思路，标签解析之后用自定义的view加载对应的数据，主动measure、layout之后draw到canvas上，获取bitmap之后加载到textview中。
- 参照table标签，解析每个tb tr标签，得到每行每列的数据，用tableLayout控件加载，主动measure、layout之后draw到bitmap上即可得到表格图片。HtmlTagHandler文件中endTable函数，进行表格处理。

##JLaTexMath解析
- 在自定义的HtmlFormulaImageGetter中，loadBitmap函数即为依靠JLaTexMath解析公式的入口。
- JLaTexMath的assets目录中有xml对照表，所有支持的公式符号均在此处。

##后续思路
- JLaTexMath如果无法解析，将会得到一张空bitmap，再让mathview来解析，获取解析之后的图片。

##对照表
> 目前所支持解析latex标签的对照表，暂时不支持排版标签。
```
<FormulaSettings>
	...
	<CharacterToFormulaMappings>
	  ...
	  <Map char="&#8799;" formula="\stackrel{\scalebox{0.75}{\mathrm{?}}}{=}"/>
